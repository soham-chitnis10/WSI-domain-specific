...
class TransLayer(nn.Module):
    ...
    def forward(self, x, return_attn=False):
        if return_attn:
            x1, A = self.attn(self.norm(x),return_attn=return_attn)
            x = x + x1
            return x, A
...
class TransMIL(nn.Module):
    def __init__(self, n_classes, device,feature_dim=1024):
        super(TransMIL, self).__init__()
        self.pos_layer = PPEG(dim=512)
        self.feature_dim = feature_dim
        if self.feature_dim!=1024:
            self._fc0 = nn.Sequential(nn.Linear(self.feature_dim,1024), nn.ReLU())
    ...
    def forward(self, **kwargs):
        ...
        if "return_attn" in kwargs.keys():
            if kwargs["return_attn"]:
                h, A = self.layer2(h,return_attn=kwargs["return_attn"])
            else:
                h = self.layer2(h, False) #[B, N, 512]
        else:
            h = self.layer2(h, False) #[B, N, 512]
        ...
        results_dict = {'logits': logits, 'Y_prob': Y_prob, 'Y_hat': Y_hat, 'A': A}
